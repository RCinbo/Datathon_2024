...
colnames(interventions) <- str_replace_all(colnames(interventions), " ", "_") %>%
  str_remove_all("\\(") %>%
  str_remove_all("\\)")
...
cardiac_arrest <- interventions %>%
  filter(str_detect(EventType_Trip, regex("cardiac arrest",
                                          ignore_case = TRUE)))
load(paste0(datafolder, "/cardiac_arrest_bxl.Rdata")) # load the brussels cardiac arrests
col_ca <- colnames(cardiac_arrest)
cardiac_arrest <- plyr::rbind.fill(
  cardiac_arrest,
  interventions_bxl %>%
    dplyr::select(col_ca[col_ca %in% colnames(interventions_bxl)])) %>%
  mutate(death = 1*(!is.na(Abandon_reason) &
                      (Abandon_reason == "Overleden" |
                         Abandon_reason == "Dood Ter Plaatse")))
```
...
Most heart attacks are classified in the highest priority categories *N0* or *N1*
...
cardiac_arrest_sf$distance_closest <- distances
